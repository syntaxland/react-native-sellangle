...
import { getBuyerCreditPoint } from "../../redux/actions/creditPointActions";
...
const GetBuyerCreditPoint = () => {
  const dispatch = useDispatch();

  const getBuyerCreditPointState = useSelector((state) => state.getBuyerCreditPointState);
      ...
      >
        <View style={styles.container}>
          <Text style={styles.title}>
            <FontAwesomeIcon icon={faCreditCard} /> Buyer/Received CPS List
          </Text>
          {loading ? (
            <Loader />
          ) : error ? (
            ...
            <>
              {currentItems.length === 0 ? (
                <Text style={styles.noData}>Buyer cps appear here.</Text>
              ) : (
                <ScrollView horizontal={true}>
                  <DataTable>
                    <DataTable.Header>
                      <DataTable.Title style={styles.snHeaderCell}>SN</DataTable.Title>
                      <DataTable.Title style={styles.headerCell}>CPS ID</DataTable.Title>
                      <DataTable.Title style={styles.headerCell}>Seller</DataTable.Title>
                      <DataTable.Title style={styles.headerCell}>Buyer</DataTable.Title>
                    ...
                    {currentItems.map((cps, index) => (
                      <DataTable.Row key={cps.id}>
                        ...
                        <DataTable.Cell style={styles.cell}>
                          <ScrollView horizontal>
                            <Text style={{ color: "green" }}>{formatAmount(cps.amount)}</Text>
                          </ScrollView>
                        </DataTable.Cell>
                        <DataTable.Cell style={styles.cell}>
                          <ScrollView horizontal>
                            <Text>{formatAmount(cps.buyer_old_bal)}</Text>
                          </ScrollView>
                        </DataTable.Cell>
                        <DataTable.Cell style={styles.cell}>
                          <ScrollView horizontal>
                            <Text>{formatAmount(cps.buyer_new_bal)}</Text>
                          </ScrollView>
                        </DataTable.Cell>
                        <DataTable.Cell style={styles.cell}>
                          <ScrollView horizontal>
                            {cps.is_success ? (
                              <Text style={{ color: "green" }}>
                                <FontAwesomeIcon color="green" icon={faCheckCircle} /> Yes
                              </Text>
                            ) : (
                              <Text style={{ color: "red" }}>
                                <FontAwesomeIcon color="red" icon={faTimesCircle} /> No
                              </Text>
                            )}
                          </ScrollView>
                        </DataTable.Cell>
              ...
              <View style={styles.pagination}>
                <Pagination currentPage={currentPage} totalPages={totalPages} paginate={handlePagination} />
              </View>
            </>
          )}
        </View>
