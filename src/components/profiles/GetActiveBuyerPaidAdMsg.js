...
const GetActiveBuyerPaidAdMsg = () => {
  ...
  const { loading, activeBuyerPaidAdMessages, loadingError } =
  ...
  const handleReplyBuyer = (message) => {
    ...
    navigation.navigate("Buyer Paid Ad Message", {
      id: message.id,
      ...queryParams,
    });
  };
  ...
  const renderItem = ({ item: message }) => (
    <View style={styles.messageItem}>
      ...
      <Text style={styles.subtitle}>
        <FontAwesomeIcon icon={faUser} /> {message?.paid_ad_seller_username}
      </Text>
      <RenderHtml
        contentWidth={300}
        source={{
        ...
        {currentItems?.length > 0 && profile.is_marketplace_seller && (
          <View>
            <Text style={styles.header}>
              <FontAwesomeIcon icon={faMessage} /> Buyer Paid Ad Inbox
            </Text>
